	...
	m_shadervalue(0.0f)
{
	...
	m_commonCtrl.addToggle((S32*)&m_viewpoint, 3,							FW_KEY_NONE,	"Light 3 viewpoint" );
	...
	m_lights.push_back(std::pair<LightSource, ShadowMapContext>());
	m_lights.push_back(std::pair<LightSource, ShadowMapContext>());
	m_lights[2].first.setStaticPosition(Vec3f(0, 0.2, -0.1));
	m_lights[2].first.setOrientation(getOrientation(-Vec3f(0, 1, 0), Vec3f(1, 0, 0)));
...
bool App::handleEvent(const Window::Event& ev)
{
	...
	if (m_timeron) {
		m_lampangle += m_lamptimer.end();
	}

	if (m_stimeron) {
		m_shadervalue += m_shadertimer.end();
	}
...
void App::renderFrame(GLContext* gl)
{
	...
	LightSource& c = m_lights[2].first;
	...
	c.setPosition(c.getStaticPosition());
	c.setOrientation(getOrientation(-c.getPosition(), Vec3f(0.0f, 1.0f, 0.0f)));
...
void App::renderWithNormalMap(GLContext* gl, Mat4f worldToCamera, Mat4f projection)
{
	...
	Random r;
	float x = (FW::cos(m_shadervalue * FW::pow(FW::sin(m_shadervalue * m_shadervalue), 0.2f)) + 1) / 2.0f;
	float y = m_shadervalue == 0 ? 1.0f : 1.0f / (1 + FW::exp2(- FW::sin(m_shadervalue) / (m_shadervalue)));	
	float z = m_stimeron ? r.getF32() : 1.0f;
	...
	gl->setUniform(m_shaderProgram->getUniformLoc("timer"), m_shadervalue);
	gl->setUniform(m_shaderProgram->getUniformLoc("value"), Vec3f(x, y, z));
	...
	// Before: world space, after: camera space

	for ( int i = 0; i < nlights; ++i )
	{
		lightDir[i] = Mat3f::rotation(Vec3f(0, 1, 0), m_lampangle) * ((worldToCamera * Vec4f( lightDir[i].normalized(), 0.0f )).getXYZ());
	}
